...
s$.fn.spin = function(opts) {
    ...
    this.each(function() {
        ...
        if (data.spinner) {
            data.spinner.stop();
            delete data.spinner;
            if (opts == modal_opts) s$("#spin_modal_overlay").remove();
            return this;
        }

        var spinElem = this;
        if (opts == modal_opts){
            s$('body').append('<div id="spin_modal_overlay" style="background-color: rgba(0, 0, 0, 0.6); width:100%; height:100%; position:fixed; top:0px; left:0px; z-index:'+(opts.zIndex-1)+'"/>');
            spinElem = s$("#spin_modal_overlay")[0];
        }
        data.spinner = new Spinner(s$.extend({color: sthis.css('color')}, opts)).spin(spinElem);
    });
    return this;
};

function toggleSpin() {			
	s$('#spinner-div').spin("modal");			
}

function stopSpin() {
	var data = s$("#spinner-div").data();	
	if(data.spinner) {
		s$("#spinner-div").spin("modal");
	}
}